...
<template>
  ...
  // Base properties
  const baseProps: Omit<MotionProp, 'value'>[] = [
  ...
  // Move to a random X/Y within their ranges
  function move() {
    const xProp = motionProps.find(p => p.key === 'x')!
    const yProp = motionProps.find(p => p.key === 'y')!
    const rand = (r: { min: number; max: number }) =>
      Math.random() * (r.max - r.min) + r.min
  
    xProp.value.value = rand(xProp.range)
    yProp.value.value = rand(yProp.range)
  }
  
  // Reset all back to their minimums
  function replay() {
