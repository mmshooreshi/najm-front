...
<template>
  <div class="min-h-screen w-full relative">
    ...
    <div :dir="isRTL ? 'rtl' : 'ltr'" class="relative min-h-screen pt-16 pb-24 bg-najmback">
      ...
      <div v-if="primaryCtaLabel || secondaryCtaLabel" class="fixed inset-x-0 bottom-0 z-40 bg-white/70 backdrop-blur-md border-t border-gray-200/60 py-3 shadow-md">
        <div class="w-full px-4 md:px-8">
          <div class="flex flex-row gap-3 justify-center max-w-md mx-auto">
            <NuxtLink
              v-if="primaryCtaLabel"
              :to="primaryCtaHref"
              class="flex-1 h-11 px-6 rounded-xl bg-najmgreen hover:bg-emerald-800 text-white text-xs sm:text-sm font-bold flex items-center justify-center transition shadow-sm text-center"
            >
              {{ primaryCtaLabel }}
            </NuxtLink>
            <NuxtLink
              ...
              :to="secondaryCtaHref"
              class="flex-1 h-11 px-6 rounded-xl border border-gray-300 bg-white hover:bg-gray-50 text-gray-800 text-xs sm:text-sm font-bold flex items-center justify-center transition text-center"
            >
              {{ secondaryCtaLabel }}
            </NuxtLink>
          </div>
