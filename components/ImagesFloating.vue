...
const verticalAmplitude = ref(70)         // pixels
const verticalSpeedFactor = ref(1.5)         // multiplier for vertical oscillation
...
const handleTouchStart = (event) => {
    ...
    lastTouchX.value = event.touches[0].clientX;
    lastTouchTime.value = performance.now();
    isRealSwipe.value = false; // 🔥 Reset swipe detection
};
const handleTouchMove = (event) => {
    const currentX = event.touches[0].clientX;
    const currentTime = performance.now();
...
const handleTouchEnd = () => {
    ...
    if (!isRealSwipe.value) {
        swipeVelocity.value = 0; // 🔥 If no swipe happened, prevent inertia
    } else {
        swipeInertiaActive.value = true; 
    }
