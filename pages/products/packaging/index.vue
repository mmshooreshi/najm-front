<!-- pages/products/packaging/index.vue -->
<template>
  <div :dir="isRTL ? 'rtl' : 'ltr'" class="min-h-screen pt-24 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-10 sm:space-y-12 select-none font-sans">
    ...
    <div class="flex items-center justify-between gap-4 flex-wrap">
      ...
      >
        <span>{{ isRTL ? 'استعلام قیمت و ماکت ۳D' : 'Request 3D Mockup' }}</span>
      </NuxtLink>
    </div>

    <!-- Products Grid -->
    <div v-if="loading" class="p-16 text-center text-gray-400 text-xs font-d4">
      {{ isRTL ? 'در حال بارگذاری بسته‌بندی‌ها...' : 'Loading packaging catalog...' }}
    </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <article
        ...
        :key="item.id"
        class="bg-white rounded-3xl overflow-hidden shadow-xs border border-gray-100 flex flex-col justify-between hover:shadow-lg hover:border-emerald-500/30 transition-all duration-300 group"
      ...
      >
        ...
        <div class="relative h-56 bg-gray-50 flex items-center justify-center p-4 overflow-hidden">
          ...
          <span class="absolute top-3 right-3 px-2.5 py-1 rounded-full text-[10px] font-bold bg-white/90 backdrop-blur-md text-najmgreen border border-gray-200 font-d4">
        ...
        <div class="p-6 flex-1 flex flex-col justify-between space-y-4">
          <div class="space-y-2">
            ...
            <div class="flex flex-wrap gap-1.5 pt-0.5">
              ...
              <span v-if="item.coating" class="px-2 py-0.5 rounded-md bg-emerald-50 text-[10px] text-najmgreen font-d4 border border-emerald-500/20">
                {{ item.coating }}
              </span>
          ...
          <div class="flex items-center gap-3 pt-4 border-t border-gray-100">
            <NuxtLink
              :to="`/products/packaging/${item.slug}`"
            ...
            >
              {{ isRTL ? 'مشاهده مشخصات فنی' : 'View Specifications' }}
            </NuxtLink>
...
const localizedHeader = computed(() => {
  if (activeLang.value === 'en') {
    return {
      badge: 'Industrial Packaging & Box Making',
      title: 'Luxury Rigid Boxes, Folding Cartons & Corrugated Boxes',
      description: 'Engineered packaging solutions for pharmaceuticals, saffron, cosmetics, and export produce with automated Heidelberg production lines.'
    }
  }
  if (activeLang.value === 'ar') {
    return {
      badge: 'صناعة التغليف والعلب الفاخرة',
      title: 'علب الهاردبوكس، الكرتون الصحي والكرتون المصفح للتصدير',
      description: 'حلول تغليف متكاملة للصناعات الدوائية، الزعفران، مستحضرات التجميل والأغذية وفق المعايير العالمية.'
    }
  }
  return {
    badge: 'صنعت بسته‌بندی، جعبه‌سازی و هاردباکس',
    title: 'تولید انواع هاردباکس، جعبه‌های ایندربرد دارویی و کارتن‌های لمینتی',
    description: 'خطوط اتوماتیک تولید جعبه‌های لوکس مگنتی، جعبه‌های بهداشتی فودگرید و کارتن‌های ۵ لایه صادراتی با بالاترین استانداردهای کیفی.'
  }
})
...
const categories = computed(() => {
  if (activeLang.value === 'en') {
    return [
      { key: 'all', label: 'All Packaging' },
      { key: 'hardbox', label: 'Rigid Boxes' },
      { key: 'folding_carton', label: 'Folding Cartons' },
      { key: 'laminated_carton', label: 'Laminated Cartons' },
      { key: 'shopping_bag', label: 'Shopping Bags' }
    ]
  }
  if (activeLang.value === 'ar') {
    return [
      { key: 'all', label: 'جميع العلب' },
      { key: 'hardbox', label: 'هاردبوكس فاخر' },
      { key: 'folding_carton', label: 'علب كرتونية صحية' },
      { key: 'laminated_carton', label: 'كرتون مصفح' },
      { key: 'shopping_bag', label: 'أكياس التسوق' }
    ]
  }
  return [
    { key: 'all', label: 'همه بسته‌بندی‌ها' },
    { key: 'hardbox', label: 'هاردباکس لوکس' },
    { key: 'folding_carton', label: 'جعبه ایندربرد دارویی' },
    { key: 'laminated_carton', label: 'کارتن لمینتی صادراتی' },
    { key: 'shopping_bag', label: 'ساک دستی و بگ' }
  ]
})
...
const filteredItems = computed(() => {
  return items.value.filter((it) => {
    if (it.division && it.division !== 'packaging') return false
...
async function fetchProducts() {
  loading.value = true
  try {
    const res: any = await $fetch('/api/products?division=packaging')
