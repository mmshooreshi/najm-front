...
function bgUtility (card: FeatureCard) {
  ...
  if (card.id === 1) return 'bg-[#E9E9E1] bg-cover bg-no-repeat bg-card-1-m md:bg-card-1-d'
  if (card.id === 2) return 'scale-x-[-1] bg-[#E9E9E1] !bg-cover bg-no-repeat bg-card-2-m md:bg-card-2-d'
  ...
  <section dir="rtl" class="mx-auto max-w-screen-xl  py-16">
    ...
    <div class="grid gap-1 md:grid-cols-3 md:gap-4">
      <article
        v-for="card in cards"
        :key="card.id"
        class="relative flex flex-col overflow-hidden rounded-3xl p-6 min-h-[200px] md:min-h-[300px]"
        :class="[
          'min-h-[200px]',               // mobile minimum
      ...
      >
        ...
        <header class="relative z-10 flex  flex-col justify-between gap-3 h-full">
          ...
          <button
            class="flex w-[181px] items-center justify-center gap-2 rounded-[15px] bg-white px-6 py-3 text-[14px] font-semibold leading-[21px]"
          >
            {{ card.buttonLabel }}
          </button>
        </header>
