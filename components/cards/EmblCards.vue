...
const getDotClasses = (index) => {
  ...
  if (isSelected) {
    ...
    if (isEdgeDot && props.cards.length > MAX_VISIBLE_DOTS && !isEndorStart) {
      classes += ' w-1.5 h-1.5 opacity-60'
    } else {
      classes += ' w-2 h-2'
    }
  }
...
onMounted(() => {
  ...
  window.addEventListener('admin:motion-global-toggle', onGlobalToggle)

  onBeforeUnmount(() => {
    window.removeEventListener('admin:motion-element-freeze', onFreeze)
    window.removeEventListener('admin:motion-global-toggle', onGlobalToggle)
  })
})
</script>
