...
let inMemoryProducts: any[] = [...initialSeedProducts]
...
function ensureStorageDir() {
  try {
    ...
    if (!fs.existsSync(PRODUCTS_FILE_PATH)) {
      fs.writeFileSync(PRODUCTS_FILE_PATH, JSON.stringify(initialSeedProducts, null, 2), 'utf-8')
    }
  } catch (err) {}
}
...
export function writeLocalProducts(items: any[]) {
  inMemoryProducts = items
  try {
    ensureStorageDir()
    fs.writeFileSync(PRODUCTS_FILE_PATH, JSON.stringify(items, null, 2), 'utf-8')
  } catch (err) {}
}

export async function getAllProducts(): Promise<any[]> {
  try {
    const res: any = await $fetch(`${PB_SERVER_URL}/api/collections/products/records`, {
    ...
    if (res?.items && res.items.length > 0) {
      const items = res.items.map((p: any) => {
        ...
        return {
          ...
          image: p.media?.[0] ? `${PB_SERVER_URL}/api/files/products/${p.id}/${p.media[0]}` : (p.image || '/images/sections/cards/01.png'),
          gallery: Array.isArray(p.media) ? p.media.map((m: string) => `${PB_SERVER_URL}/api/files/products/${p.id}/${m}`) : [],
      ...
      // Merge PB items with local storage
...
export async function saveProduct(item: any): Promise<any> {
  const localItems = readLocalProducts()
  const savedItem = { ...item }

  if (!savedItem.id) {
    savedItem.id = `prod-${Date.now()}`
    localItems.unshift(savedItem)
  } else {
    const idx = localItems.findIndex(p => p.id === savedItem.id)
    if (idx !== -1) {
  ...
  writeLocalProducts(localItems)

  // Sync to PocketBase in background
  try {
    if (savedItem.id && !savedItem.id.startsWith('pkg-') && !savedItem.id.startsWith('prt-') && !savedItem.id.startsWith('prod-')) {
      await $fetch(`${PB_SERVER_URL}/api/collections/products/records/${savedItem.id}`, {
        method: 'PATCH',
...
export async function removeProduct(id: string): Promise<boolean> {
  const localItems = readLocalProducts()
  const filtered = localItems.filter(p => p.id !== id && p.slug !== id)
  writeLocalProducts(filtered)

  // Delete from PocketBase in background
  try {
    if (!id.startsWith('pkg-') && !id.startsWith('prt-') && !id.startsWith('prod-')) {
      await $fetch(`${PB_SERVER_URL}/api/collections/products/records/${id}`, {
        method: 'DELETE',
        headers: { Authorization: PB_SUPERUSER_TOKEN },
        timeout: 3000
      }).catch(() => null)
