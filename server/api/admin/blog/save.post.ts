// server/api/admin/blog/save.post.ts
...
export default defineEventHandler(async (event) => {
  ...
  try {
    if (body.id && !body.id.startsWith('mock-')) {
      const res = await $fetch(`${PB_SERVER_URL}/api/collections/blog/records/${body.id}`, {
        method: 'PATCH',
      ...
      const res = await $fetch(`${PB_SERVER_URL}/api/collections/blog/records`, {
        method: 'POST',
        headers: { Authorization: PB_SUPERUSER_TOKEN, 'Content-Type': 'application/json' },
        body
      })
